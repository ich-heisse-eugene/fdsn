...
class Spec():
    def __init__(self, file_iraf):
        ...
        print(f"Spectrum of {objname} spans from {self.w0:.3f} to {self.wn:.3f} {self.u} in {self.norders} orders")
        return None
...
# End of
    ...
    def stitchOutput(self, event):
        ...
        if self.button_merge.color != "black":
            ...
            self.fig.canvas.draw_idle()
            stitched_fname = self.merged_filename.text.strip().replace(".fits", "_I.dat")
